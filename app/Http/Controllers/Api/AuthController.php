...
class AuthController extends Controller
{
  ...
  public function register(RegisterRequest $request)
  {
    ...
    try {
      ...
      $data = ['id' => $user->id, 'otp' => $verified_code, 'email' => $user->email, 'image' => $user->image];
  ...
  public function confirmCode(ConfirmPhoneRequest $request)
  {
    ...
    if ($user) {
      ...
      $token = $user->createToken('study')->accessToken;
  ...
  public function userLogin(LoginRequest $request)
  {
    ...
    if (!$user_data->is_active) {
      ...
      $token = $user_data->createToken('study')->accessToken;
  ...
  public function userSocialLogin(SocialRequest $request)
  {
    ...
    $token = $user->createToken('study', ['user'])->accessToken;
    $data = data_set($user, 'token', $token);
    $user = SimpleResource::make($user);
    return $this->helper->responseJson('success', trans('api.user_logged_success'), 200, ['user' => $user]);
  ...
  public function resendOtp(ResendOtpRequest $request)
  {
    ...
    $user->update(['otp' => $otp]);
    $data = ['email' => $request->email, 'otp' => $user->otp];
  ...
  public function sendCode(SendCodeRequest $request)
  {
    ...
    //   "otp" => $user->otp
    // ]);
    $data = ['email' => $user->email, 'otp' => $user->otp];
  ...
  /**
   * 2:Check Code correct Or Not To Send email
   */
  public function checkCode(CheckCodeRequest $request)
  {
    ...
    $data = ['email' => $user->email,'otp' => (int) $user->otp];
