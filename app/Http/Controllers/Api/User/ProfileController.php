...
use App\Http\Resources\Api\User\{SimpleResource,PostResource};
...
class ProfileController extends Controller
{
  ...
  public function editPassword(EditPasswordRequest $request)
  {
    ...
    try {
      ...
      $data = ['user_id' => $user->id, 'image' => $user->image, 'fullname' => $user->fullname];
  ...
  public function showProfile()
  {
    ...
    $data = data_set($user, 'token', $token);
    $feeds=$this->ShowFeedsOfTheUser("paginate");

    return response()->json([
      'result' => 'success',
      'message' => trans('api.auth_data_retreive_success'),
      'status' => 200,
      'data' =>
        [
          'user' => $user,
          'feeds'=>PostResource::collection($feeds)
        ]
    ]);
